    ...
    public partial class kabulRed : Form
    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            while(dongu>0 && Convert.ToInt32(tablo[2, dongu-1]) < miktar)
            {
                ...
                int hold = 0;
                if (reader2.Read()) hold = Convert.ToInt32(reader2["cuzdan"].ToString());
                reader2.Close();

                SqlCommand guncelle = new SqlCommand("Update basvuru set cuzdan = @cuzdan Where kullaniciAdi = @KA", baglanti);
                guncelle.Parameters.AddWithValue("@cuzdan", (hold + (Convert.ToInt32(tablo[2, dongu-1]) * Convert.ToInt32(tablo[3, dongu-1]))));
                guncelle.Parameters.AddWithValue("@KA", lblUserName.Text);
                guncelle.ExecuteNonQuery();
                ...
                rapor.Parameters.AddWithValue("@Alici", lblKullaniciAdi.Text);
                rapor.Parameters.AddWithValue("@satici", tablo[0,dongu-1]);
                ...
                MessageBox.Show(tablo[0, dongu - 1] + "'den," + tablo[1, dongu - 1] + " aldık ve " + (Convert.ToInt32(tablo[2, dongu - 1]) * Convert.ToInt32(tablo[3, dongu - 1]))+"₺ tuttu.");
